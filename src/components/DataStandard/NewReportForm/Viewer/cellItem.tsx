...
import { cellsDataType, coord } from '../types';
...
interface ICellItemProps {
  ...
  type: string;
  cellsData: cellsDataType;
  coordinate: coord;
  selectValue: string;
  fieldId: string;
  onValuesChange?: (field: string, value: any) => void;
  onCancel: () => void;
  writeData: (text: string, value: string, fieldId: string) => void;
}
...
const CellItem: React.FC<ICellItemProps> = (props) => {
  ...
  useEffectOnce(() => {
    if (props.data[props.field.id] == undefined && props.field.options?.defaultValue) {
      const key = excelCellRef(props.coordinate);
      const data: cellsDataType = deepClone(props.cellsData);
      data[key] = props.field.options?.defaultValue;
      props.onValuesChange?.apply(this, [props.fieldId, data]);
    }
  });
    ...
    <Modal
      ...
      onOk={() => {
        if (props.type === 'primary') {
          ...
          data[key] = title;
          props.onValuesChange?.apply(this, [props.fieldId, data]);
          props.writeData(title, value, props.field.id);
        } else {
          props.writeData(title, value, props.field.id);
        }
        props.onCancel();
