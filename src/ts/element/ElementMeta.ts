...
export type PrimitiveType = "string" | "number" | "boolean" | "date"| "json";
export type ComplexType = "enum" | "array" | "object" | "type";
export type DataType = PrimitiveType | ComplexType;


export type ElementType = "Element" | 'Document' | "Container" | 'Template';
export type ElementTreeNodeType = ElementType | 'Slot' | 'ArraySlot' ;
export enum ElementTreeNodeName {
  Element = '元素',
  Container = '容器',
  Slot = '插槽',
  ArraySlot = '数组插槽',
...
export interface StringTypeMeta extends TypeMetaBase<'string'> {
  inputType?: string;
}
export interface JsonTypeMeta extends TypeMetaBase<'json'> {
  inputType?: string;
}
export interface NumberTypeMeta extends TypeMetaBase<'number'> {
  min?: number;
  max?: number;
}

export interface BooleanTypeMeta extends TypeMetaBase<'boolean'> {

}
export interface DateTypeMeta extends TypeMetaBase<'date'> {
...
export interface ElementMeta {
  ...
  /** 定义附加属性，附加到子元素上面 */
  attachProps?: Dictionary<TypeMeta>;
  /** 组件中文名 */
  label: string;
  /** 元素类型 */
  type: ElementType;
  ...
  /** 子元素筛选器 */
  childrenFilter?: string[] | ((element: ElementMeta & { name: string }) => boolean);
}
