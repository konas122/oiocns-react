...
import AddElementModal from '../pageBuilder/design/AddElementModal';
import { removeElement } from '../pageBuilder/design/config/ElementProps';
import cls from '../pageBuilder/design/tree.module.less';
...
const TreeManager: React.FC<{}> = () => {
  ...
  function canAdd({ typeName, item }: any) {
    return (
      ['ArraySlot', 'Container', 'Document'].includes(typeName) &&
      !['Table', 'ListTable'].includes(item.kind)
    );
  }
  const open = () => {
    setCenter(
